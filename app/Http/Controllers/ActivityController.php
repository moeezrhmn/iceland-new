...
//        }
        ...
        return view("activities.checkout",compact('item'));
    ...
    public function detail(Request $request)
    {
     ...
     // dd($item);
       ...
       // return view("activities.detailed",$item, compact('activity_detail','lat','long','name','reviews','item','favoruite'));
        ...
        return view("activities.detailed",compact('lat','long','name','reviews','item','favoruite'));
    }
...
//dd($restaurants);
  ...
  // dd($places);
    ...
    public function getActivitiesBySubCat($slug)
    {
        ...
        ->with(['reviews_avg' => function ($query) {
            $query->where('category_id', '=', 3)
                  ->select('instance_id', \DB::raw('AVG(rating) as rating'))
                  ->groupBy('instance_id');
        }])
        ->with(['favoruite' => function ($query) {
            $query->where('category_id', '=', 3);
        ...
        $listing->whereHas('subcategories', function ($query) use ($sub_cat_id) {
            $query->where('category_id', '=', 3)
                  ->where('subcategory_id', '=', @$sub_cat_id->id);
        });
        ...
        if (!empty($places) && sizeof($places) > 0) {
            ...
            return view('activities/subcatListing', compact('places', 'sub_catid', 'subcat_place','map_list','marker_list'))->with('sub_cat_detail', $sub_cat_id);
        }else{
