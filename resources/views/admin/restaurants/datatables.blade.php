...
@section('create_page_url', route('restaurants.create'))
@section('sub-content')
    <?php
    $tableHeadings = [
        'Restaurant Name',
                ...
                <div class="col-md-3 col-xs-5 hotel_places_listing">
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="subcategory_id" onchange="$('#listing').DataTable().draw()"  data-placeholder="Select Sub Categories" name="subcat_id">
                            @if(isset($subcategory) && sizeof($subcategory))
                                <option value="0">Select Sub Categories</option>
                                @foreach($subcategory as $row)
                                    <option  value="{{$row->id}}" >{{ $row->cat_name }}</option>
                                    {{--<option  @if(isset($_GET['subcat_id']) && $_GET['subcat_id'] ==$row->id) selected="selected"--}}
                                    {{--@endif data-{{ $row->id }}="{{ $row->cat_name }}">{{ $row->cat_name }}</option>--}}
                                @endforeach
                            @endif
                        </select>
                    </div>
                                ...
                                <div class="m-dropdown__content">
                                    <ul class="m-nav" >
                                        ...
                                        <li class="m-nav__item">
                                            <a href="{{url('admin/places/import-excel')}}" class="m-nav__link">
    ...
    <script type="application/javascript">
        ...
        jQuery(document).ready(function ($) {
           ...
           "lengthMenu": [[10, 25, 50,100,250,500,1000, -1], [10, 25, 50,100,250,500,1000, "All"]],
                ...
                processing: true,
                serverSide: true,
                //ajax: '{{url("admin/restaurant-listing") }}',
                ajax: {
                    url: '{{ url('admin/restaurant-listing') }}',
                    //type:'post',
                    data: function (d) {
                ...
                columns: [
                    ...
                    {data: 'subcat_name', name: 'subcat_name', defaultContent:'N/A' },
